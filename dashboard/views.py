...
def invitations_view(request, user_id=None):
    ...
    referrals = CustomUser.objects.filter(referred_by=user,registration_fee_paid=True).reverse()  # all people referred by this user
...
def referral_children(request, user_id):
    user = get_object_or_404(CustomUser, id=user_id)
    referrals = CustomUser.objects.filter(referred_by=user,registration_fee_paid=True).reverse()
